...
void Torneo::armarGrupos() {
    ...
    for (int b = 0; b < 4; ++b) {
        tamaños[b] = base + (b < restoB ? 1 : 0);
        bombos[b] = new Equipo*[tamaños[b]];
        for (unsigned short int i = 0; i < tamaños[b]; ++i) {
            bombos[b][i] = resto[acum++];
        }
    }
    // Añadir anfitrion al bombo 1 (1ra pocision)
    Equipo** nuevoBombo1 = new Equipo*[tamaños[0]+1];
    nuevoBombo1[0] = anfitrion;
    for (unsigned short int i = 0; i < tamaños[0]; ++i){
    ...
    for (int bombo = 0; bombo < 4; ++bombo) {
        ...
        Equipo** disponibles = new Equipo*[tamaños[bombo]];
        for (unsigned short int i = 0; i < tamaños[bombo]; ++i){
...
void Torneo::transicionADieciseisavos() {
    ...
    unsigned short int numMejoresTerceros = totalGrupos - (totalGrupos / 2); // para 12 grupos son 8
